...
const LikedPostPage = () => {
  // State Management
  const [likedPosts, setLikedPosts] = useState<CollectionItem[]>([]);
    ...
    const fetchLikedPosts = async () => {
      ...
      try {
        ...
        const response = await axios.get(
          `/api/inscriptions/getLikedPosts?userId=${session.user._id}`
        );

        if (response.data.success) {
          setLikedPosts(response.data.likedPosts);

          // Initialize saved items if available
          if (response.data.savedItemIds) {
            setSavedItemIds(response.data.savedItemIds);
          }
        } else {
